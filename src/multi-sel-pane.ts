...
import {
  $$,
  $$byTag, $byClass, hasClass, remeveBookmark, showMenu,
} from './client';
...
export class MultiSelPane extends HTMLElement implements ISubscribeElement {
  ...
  show(value: { leafs?: boolean, tabs?: boolean, history?: boolean }) {
    const [, show] = Object.entries(value).find(([key]) => key === this.#className) || [];
